...
- glossary (preserve these term choices): pstack: pstack；Dune: Dune；Grok Bot: Grok Bot；Bugbot: Bugbot；feature map: 功能地图（Feature Map）；heap snapshot: 堆快照；formal verification: 形式化验证；invariant: 不变量；paved path: 标准路径；outer loop: 外环
- 执行策略：精修翻译（refined）。
- 先做术语与语气分析，再完成译文。
...
---
Lauren Tan (@poteto). Cleaned verbatim transcript of the supplied 38:02 video. Paragraph timestamps are navigation aids. Filler words, immediate repetitions, punctuation, and clear ASR errors have been cleaned; the order and substance of the speech are retained. Section headings are editorial, not spoken.
